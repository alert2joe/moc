...
export default class BasePage {
  ...
  async isNotExist(locator: WebDriverLocator){
   try {
    await this.driver.wait(until.elementLocated(locator), 500)
    return false;
   } catch (err) {
    return true;
   }
  ...
  async is404(){
       try {
        await this.driver.wait(until.elementLocated(By.css('title')), 500)
        return false;
       } catch (err) {
        return true;
       }
